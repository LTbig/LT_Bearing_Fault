...
def filter(signal_1, signal_2):
    ...
    return Min_Max_Scaling_2(current_multiplication)
...
def ready_data():
    ...
    PU_data_n = filter(PU_data_n_2, PU_data_n_1)
    PU_data_in = filter(PU_data_in_2, PU_data_in_1)
    PU_data_out = filter(PU_data_out_2, PU_data_out_1)
    PU_data_inout = filter(PU_data_inout_2, PU_data_inout_1)
    PU_data_outin = filter(PU_data_outin_2, PU_data_outin_1)
    '''
    2.处理轴承数据
    '''
    # 切割数据
