...
from read_multi_fault_data_ALL_Feature_fusion_PU_N09_M07_F10_phase_current_and_vibration import ready_data
...
def Confusion_matrix_plt(confusion, epoch):
    ...
    # 每1轮的混淆矩阵的标题
    plt.title('Confusion matrix_{}_N09_M07_F10'.format(epoch))
...
def get_paramer_matrix(matrix):
    ...
    OR_FP = (matrix[1, 0] + matrix[2, 0] + matrix[3, 0] + matrix[4, 0]) / All
    OR_FN = (matrix[0, 1] + matrix[0, 2] + matrix[0, 3] + matrix[0, 4]) / All
    ...
    IR_FP = (matrix[0, 1] + matrix[2, 1] + matrix[3, 1] + matrix[4, 1]) / All
    IR_FN = (matrix[1, 0] + matrix[1, 2] + matrix[1, 3] + matrix[1, 4]) / All
    ...
    ORIR_TP = matrix[2, 2] / All
    ORIR_FP = (matrix[0, 2] + matrix[1, 2] + matrix[3, 2] + matrix[4, 2]) / All
    ORIR_FN = (matrix[2, 0] + matrix[2, 1] + matrix[2, 3] + matrix[2, 4]) / All
    ORIR_TN = 1 - ORIR_TP - ORIR_FP - ORIR_FN
    ORIR_precision = ORIR_TP / (ORIR_TP + ORIR_FP)
    ORIR_recall = ORIR_TP / (ORIR_TP + ORIR_FN)
    ORIR_F1 = 2 * ORIR_precision * ORIR_recall / (ORIR_precision + ORIR_recall)

    print("==========ORIR result==========")
    print(f'ORIR_TP:{ORIR_TP}')
    print(f'ORIR_FP:{ORIR_FP}')
    print(f'ORIR_TN:{ORIR_TN}')
    print(f'ORIR_FN:{ORIR_FN}')
    print(f'ORIR_precision:{ORIR_precision}')
    print(f'ORIR_recall:{ORIR_recall}')
    print(f'ORIR_F1:{ORIR_F1}')

    IROR_TP = matrix[3, 3] / All
    IROR_FP = (matrix[0, 3] + matrix[1, 3] + matrix[2, 3] + matrix[4, 3]) / All
    IROR_FN = (matrix[3, 0] + matrix[3, 1] + matrix[3, 2] + matrix[3, 4]) / All
    IROR_TN = 1 - IROR_TP - IROR_FP - IROR_FN
    IROR_precision = IROR_TP / (IROR_TP + IROR_FP)
    IROR_recall = IROR_TP / (IROR_TP + IROR_FN)
    IROR_F1 = 2 * IROR_precision * IROR_recall / (IROR_precision + IROR_recall)

    print("==========IROR result==========")
    print(f'IROR_TP:{IROR_TP}')
    print(f'IROR_FP:{IROR_FP}')
    print(f'IROR_TN:{IROR_TN}')
    print(f'IROR_FN:{IROR_FN}')
    print(f'IROR_precision:{IROR_precision}')
    print(f'IROR_recall:{IROR_recall}')
    print(f'IROR_F1:{IROR_F1}')

    NORMAL_TP = matrix[4, 4] / All
    NORMAL_FP = (matrix[0, 4] + matrix[1, 4] + matrix[2, 4] + matrix[3, 4]) / All
    NORMAL_FN = (matrix[4, 0] + matrix[4, 1] + matrix[4, 2] + matrix[4, 3]) / All
    ...
    print(f'Macro-F1:{(OR_F1 + IR_F1 + ORIR_F1 + IROR_F1 + NORMAL_F1 ) / 5}')
...
class Net(nn.Module):
    def __init__(self):
        ...
        # 卷积层
        self.conv1 = nn.Conv2d(7, 16, kernel_size=5)
        ...
        # LSTM层
        self.lstm = nn.LSTM(2912, 800, batch_first=True)
...
def test():
    ...
    if test_acc == 100:
        torch.save(model, './models/N09_M07_F10_CAR_{}.pth'.format(test_acc))
