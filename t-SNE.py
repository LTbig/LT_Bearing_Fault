...
model = torch.load('models/CAR_99.48381696428571.pth',  map_location=torch.device('cpu'))
...
with torch.no_grad():
    for images, labels in dataloader:
        ...
        output = model(images)
        # labels = model(labels)
        last_hidden_layer = model.liner2.weight
...
X_tsne = tsne.fit_transform(last_hidden_layer.detach().numpy())
print(X_tsne.shape)
print(X_tsne)
# 可视化结果
