...
def filter(signal_1):
    ...
    return Min_Max_Scaling(current1_filtered)
...
def cut_data(nu_arr, number):
    ...
    for value in nu_arr:
        if i % (number + 1) != 0:
            ...
            result_cut.append(waverec((np.array(temp))))
            temp = []
        i = i + 1
    return result_cut
...
def ready_data():
    ...
    PU_N09_M07_F10_data = load_N09_M07_F10_data_phase_current_2()
    print("PU_N09_M07_F10_data_length:", len(PU_N09_M07_F10_data))
    PU_data_n = PU_N09_M07_F10_data[0]
    PU_data_in = PU_N09_M07_F10_data[2]
    PU_data_out = PU_N09_M07_F10_data[1]


    PU_data_n = filter(PU_data_n)
    PU_data_in = filter(PU_data_in)
    PU_data_out = filter(PU_data_out)
